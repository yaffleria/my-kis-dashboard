        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          const validAccounts: Account[] = parsed
            .map((item: RawEnvAccount) => {
              const accountNo = String(item.accountNo || '').replace(/[^0-9]/g, '')
              if (!accountNo) return null

              let productCode = String(item.productCode || '01').trim()
              if (productCode.length === 1) productCode = productCode.padStart(2, '0')

              const isPension = item.isPension === true || productCode === '22' || productCode === '29'
          ...
          if (currentJson !== newJson) {
            console.log('[EnvInitializer] Syncing accounts from environment variables...')
            setAccounts(validAccounts)

            // 선택된 계좌가 없거나 유효하지 않으면 첫 번째 계좌 선택
            const currentSelectedValid = validAccounts.some(
              (a) => a.accountNo === useAccountStore.getState().selectedAccountNo
            )
